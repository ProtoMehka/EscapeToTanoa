...
switch (_code) do {
    ...
    case 19: {
        ...
        if (_shift && playerSide isEqualTo west && {!isNull cursorObject} && {cursorObject isKindOf "Man"} && {(isPlayer cursorObject)} && {(side cursorObject in [civilian,east])} && {alive cursorObject} && {cursorObject distance player < 3.5} && {!(cursorObject getVariable "Escorting")} && {!(cursorObject getVariable "restrained")} && {speed cursorObject < 1}) then {
    ...
    case 34: {
        if (_shift) then {_handled = true;};
        if (_shift && playerSide in [civilian,east] && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (!(isNull objectParent player) && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","B_Heli_Transport_01_F","shounka_avalanche_noir","shounka_buggy_noir","shounka_monsteur_noir","shounka_rs6_noir","Mrshounka_a3_dodge15_civ_noir","Mrshounka_jeep_blinde_noir_mat","Mrshounka_agera_p_noir"]) then {
