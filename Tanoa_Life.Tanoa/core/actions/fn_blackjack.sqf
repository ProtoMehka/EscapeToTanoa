...
if (_number == 2) then {robberyreward = 0; hint "8! Hé ben non…";};
if (_number == 3) then {robberyreward = 0; hint "14! Vous semblez aimer perdre...";};
if (_number == 4) then {robberyreward = 0; hint "16! Votre compte en banque pleure...";};
if (_number == 5) then {robberyreward = 0; hint "12! Même ma grand-mère joue mieux!";};
if (_number == 6) then {robberyreward = 0; hint "13! Perdu à nouveau... Vraiment mauvais!";};
if (_number == 7) then {robberyreward = 10000; hint "20! Pas mal! Vous avez gagné!";};
if (_number == 8) then {robberyreward = 0; hint "10! Vous avez gagné le droit de rejouer!";};
if (_number == 9) then {robberyreward = 0; hint "11! Cela vaut-il la peine de continuer à vous enfoncer?";};
...
while {true} do {	//Timer display (TO BE REPLACED WITH A NICE GUI LAYER)
  ...
  if(player distance _vault > 5) exitWith {_toFar = true;};	if((round(_timer - time)) < 1) exitWith {};
...
switch(true) do{
  ...
  case (_toFar):	{
	   hint "Vous êtes trop loin, restez à votre place!";
     _denied1 = false;
     life_action_inUse = false;
   };

  case (!alive player):	{
     hint "Le jeu a mal tourné, vous êtes mort.";
     _denied1 = false;
     life_action_inUse = false;
  };

  case (life_istazed):	{
    hint "Vous étiez tazé, le jeu a échoué!";
    _denied1 = false;
    life_action_inUse = false;
  };
